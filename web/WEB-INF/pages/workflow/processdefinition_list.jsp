    ...
    <table id="grid" class="easyui-datagrid">
        ...
        <s:iterator value="list" var="processDefinition">
            ...
            <tr>
                ...
                <td>
                    <a onclick="window.showModalDialog('${pageContext.request.contextPath}/processDefinitionAction_viewpng?id=${id}')" class="easyui-linkbutton"
                         data-options="iconCls:'icon-search'">查看流程图
                    </a>
                    <a onclick="del('${id}')" class="easyui-linkbutton"
                       data-options="iconCls:'icon-remove'">删除
                    </a>
                </td>
            </tr>
        </s:iterator>
...
<script>
    var deltag = '${deltag}'
    if (deltag=='0') {
        $.messager.alert('提示信息', "当前的流程定义不能删除", 'warning`');
    }
    function del(id) {
        $.messager.confirm('确认信息','你确定要删除当前的流程定义吗？',function (r) {
            if (r) {
